...
with open(output_file,"w") as out:
    for dir in dirs:
        ...
        if os.path.isdir(folder):
            paml_result_model1 = folder +"/"+ "/PAML_output_model1.txt"
            paml_result_model0 = folder +"/"+ "/PAML_output_model0.txt"
            genelist = dir
            with open(paml_result_model1,"r") as input_paml, open(paml_result_model0,"r") as input_paml_model0:
                ds_tree = ""
                dn_tree = ""
                model0_freedom = ""
                model0_lnl = ""
...
#calculate p_value
                ...
                for line in input_paml:
                    if line.startswith("lnL"):
                        lnl = line
                        lnl_list = lnl.split()
                        model1_freedom = float(lnl_list[3].rstrip("):"))
                        model1_lnl = float(lnl_list[4])
                        print("model1: ", model1_freedom, model1_lnl)
                ...
                print(genelist)
                if isinstance(model1_lnl,float):
                    p_value = 1-stats.chi2.cdf((float(model1_lnl) - float(model0_lnl))*2,float(model1_freedom)-float(model0_freedom))   # 卡方检验
                    result = genelist + "\t" + dn_tree.rstrip("\n") + "\t" + ds_tree.rstrip("\n") + "\t" + w_origintree.rstrip("\n") + "\t" + w_ratio_tree +"\t" + str(model1_lnl) +"\t" +str(model0_lnl)+"\t" + str(model1_freedom) +"\t"+str(model0_freedom) +"\t" +str(p_value) +"\n"
                else:
